...
int main() {
    ...
    cout << "<";
    while (yo.size() > 1) //size가 1 -> 그냥 출력
    {
        for (int i = 0; i < K-1; i++) //K번 cur 증가
        {
            cur++;
            if (cur == yo.end()) //cur이 마지막 원소를 가리키면 
                cur = yo.begin(); // begin()으로 변환 원형 연결리스트 같이
        }
        
        cout << *cur << ", ";
        cur = yo.erase(cur); //erase하고 다음 원소를 반환
